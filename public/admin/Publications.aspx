<%@ Page Language="C#" AutoEventWireup="true" %>
    ...
    <% bool isAuthenticated=false; HttpCookie authCookie=Request.Cookies["AdminToken"]; string authToken=authCookie
        ...
        <head>
            ...
            <title>Manage Publications - ORIC Admin</title>
        ...
        <body class="bg-gray-50">
                ...
                <main class="flex-1 flex flex-col overflow-hidden">
                    ...
                    <header class="gradient-header text-white shadow-lg">
                        <div class="px-6 py-4 flex items-center justify-between">
                            <div>
                                <h1 class="text-2xl font-bold">Manage Publications</h1>
                                <p class="text-blue-100 text-sm">Upload and manage research publications</p>
                            </div>
                        ...
                        <div class="bg-white rounded-lg shadow-lg">
                            <div class="p-6">
                                <div class="flex items-center justify-between mb-4">
                                    <h2 class="text-lg font-bold text-gray-800">Publications <span
            ...
            <div id="uploadModal"
                ...
                <div class="bg-white rounded-lg max-w-md w-full p-6">
                    <h2 class="text-xl font-bold text-gray-800 mb-4">Upload Publication PDF</h2>
                    ...
                    <div id="formContainer" class="mt-4 space-y-3 hidden">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Publication Title</label>
                            <input type="text" id="pubTitle"
                                class="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-blue-600"
                                placeholder="Enter publication title" />
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Publication Date</label>
                            <input type="date" id="pubDate"
                                class="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-blue-600" />
                        </div>
                        <label class="flex items-center gap-2">
                            <input type="checkbox" id="publishPub" checked />
                    ...
                    <div class="flex gap-2 mt-6">
                        <button type="button" id="uploadBtn" onclick="uploadPublication()"
            ...
            <script>
                ...
                function handleFiles(files) {
                    ...
                    document.getElementById('pubTitle').value = file.name.replace('.pdf', '');
                ...
                async function uploadPublication() {
                    ...
                    try {
                        // Upload file
                        const uploadResponse = await fetch('http://localhost:5233/api/upload/pdf/publications', {
                            method: 'POST',
                ...
                async function loadPublications() {
                    try {
                        ...
                        document.getElementById('totalCount').textContent = `(${publications.length} Total)`;

                        if (publications.length === 0) {
                            tbody.innerHTML = '<tr><td colspan="4" class="px-4 py-8 text-center text-gray-500">No publications found</td></tr>';
                            return;
                        }

                        tbody.innerHTML = publications.map(item => {
                            const date = new Date(item.publishDate).toLocaleDateString();
                            return `
                        <tr class="border-b border-gray-200 hover:bg-gray-50 publication-row" data-title="${item.title.toLowerCase()}">
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                <button onclick="deletePublication(${item.id})" class="text-red-600 hover:text-red-800">
                    ...
                    `;
                        ...
                        document.getElementById('publicationsList').innerHTML = '<tr><td colspan="4" class="px-4 py-8 text-center text-red-500">Error loading publications</td></tr>';
                    }
                }
